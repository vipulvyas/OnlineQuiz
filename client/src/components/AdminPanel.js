...
function AdminPanel() {
  ...
  const adminUserName = 'secret_admin_name'; // fetch from database
    ...
    if (correctAnsArr.includes(correct_answer)) {
      try {
        const sendData = await axios.post(
          "http://localhost:5000/quiz/postQuiz",
          {
  ...
  return (
    adminUserName === userName ? <div className="d-flex align-items-center justify-content-center">
      <div style={qfrom}>
